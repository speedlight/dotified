...
active_theme = themes .. "/speedlight"
terminal = "terminator"
...
tags = {
       names = { " www", " cmd", " dev", " etc", " chat" },
       layout = { layouts[1], layouts[6], layouts[6], layouts[1], layouts[7] }
       --icons = { "home/.config/awesome/themes/speedlight/icons/cat.png", 
       --"home/.config/awesome/themes/speedlight/icons/cat.png", 
       --"home/.config/awesome/themes/speedlight/icons/cat.png", 
       --"home/.config/awesome/themes/speedlight/icons/cat.png" }
       }
for s = 1, screen.count() do
   ...
   --awful.tags.seticon(tags.icons[2],tags[s][2])
   --awful.tags.seticon(tags.icons[3],tags[s][3])
   --awful.tags.seticon(tags.icons[4],tags[s][4])
end
...
mytextclock = awful.widget.textclock("<span font=\"Signika 10\"> %d %b  %H:%M</span>")
...
today_color = theme.tasklist_fg_focus
calendar_width = 21
  ...
  --if (args["{count}"] > 0) then
    --if (notify_shown == false) then
      --if (args["{count}"] > 1) then 
          --notify_title = "Hay " .. args["{count}"] .. " nuevos"
          --notify_text = 'Ultimo: "' .. args["{subject}"] .. '"'
      --else
          --notify_title = "Hay nuevos mensajes"
          --notify_text = args["{subject}"]
      --end
      ...
      --notify_shown = true
    --end
    --return '<span background="#313131" font="Signika 13" rise="2000"> <span font="Signika 10">' .. args["{count}"] .. ' </span></span>'
  --else
    --notify_shown = false
    --return ""
  --end
...
function(widget, args)
    ...
    mpdicon:set_image(beautiful.widget_music_on)
		return "<span font='Signika 10'><span font='Signika 10'>" .. red .. args["{Artist}"] .. coldef .. colwhi .. " - " .. coldef .. colwhi  .. args["{Title}"] .. coldef .. " </span></span>"
	-- pause
	elseif (args["{state}"] == "Pause") then
    mpdicon:set_image(beautiful.widget_music)
		return "<span font='Signika 10'><span font='Signika 10'>" .. colwhi .. "Paused" .. coldef .. " </span></span>"
	else
    mpdicon:set_image(beautiful.widget_music)
		return ""
	end
...
vicious.register(cpuwidget, vicious.widgets.cpu, '<span font="Signika 13" rise="2000"> <span font="Signika 10">$1% </span></span>', 3)
...
vicious.register(tempwidget, vicious.widgets.thermal, '<span font="Signika 12"> <span font="Signika 10">$1°C </span></span>', 9, {"coretemp.0", "core"} )
...
function (widget, args)
  if args["{/home used_p}"] >= 95 and args["{/home used_p}"] < 99 then
    return '<span font="Signika 12" rise="2000"> <span font="Signika 10">' .. args["{/home used_p}"] .. '% </span></span>' 
  elseif args["{/home used_p}"] >= 99 and args["{/home used_p}"] <= 100 then
    naughty.notify({ title = "Atencion", text = "La particion /home casi llena!\nLibere un poco de espacio.",
    timeout = 10,
    position = "top_right",
    fg = beautiful.fg_urgent,
    bg = beautiful.bg_urgent })
    return '<span font="Signika 13" rise="2000"> <span font="Signika 10">' .. args["{/home used_p}"] .. '% </span></span>' 
  else
    return '<span font="Signika 13" rise="2000"> <span font="Signika 10">' .. args["{/home used_p}"] .. '% </span></span>' 
  end
end, 600)
...
function add_info()
    ...
    infos = naughty.notify({
        text = string.format('<span font_desc="%s">%s</span>', "Signika", cal),
      	timeout = 0,
        position = "top_right",
        margin = 10,
        height = 170,
...
function (widget, args)
  ...
  if (batstate() == 'Cable plugged' or batstate() == 'Unknown') then
    ...
    return '<span font="Signika 12"> <span font="Signika 10">AC </span></span>'
    -- critical
  ...
  elseif (args[2] <= 10 and batstate() == 'Discharging') then
    ...
    return '<span font="Signika 12"> <span font="Signika 10">' .. args[2] .. '% </span></span>'
end, 1, 'BAT1')
...
function (widget, args)
  ...
  return '<span font="Signika 12"> <span font="Signika 10">' .. args[1] .. '% </span></span>'
end, 1, "Master")

-- Net widget
netwidget = wibox.widget.textbox()
vicious.register(netwidget, vicious.widgets.net, '<span font="Signika 14" rise="2000"> <span font="Signika 10" color="#7AC82E">${eth5 down_kb}</span> <span font="Signika 7" color="#EEDDDD">↓↑</span> <span font="Signika 10" color="#46A8C3">${eth5 up_kb} </span></span>', 3)
vicious.register(netwidget, vicious.widgets.net, '<span background="#313131" font="Terminus 13" rise="2000"> <span font="Terminus 9" color="#7AC82E">${eth5 down_kb}</span> <span font="Terminus 7" color="#EEDDDD">↓↑</span> <span font="Terminus 9" color="#46A8C3">${eth5 up_kb} </span></span>', 3)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ altkey, "Control" }, "+", function ()
                                                awful.util.spawn( "ncmpcpp volume +1", false )
                                                vicious.force({ mpdwidget } )
                                             end ),
    awful.key({ altkey, "Control" }, "-", function () 
                                                awful.util.spawn( "ncmpcpp volume -1", false )
    ...
    awful.key({ modkey, 	 }, "t", 	  function () awful.util.spawn( "icedove", false ) end),
    awful.key({ modkey,        }, "d", 	  function () awful.util.spawn( "thunar", false ) end),
    awful.key({ modkey,        }, "s", 	  function () awful.util.spawn( "cb-lock", false ) end),
    awful.key({ modkey,        }, "p", 	  function () awful.util.spawn( "xscreensaver-command -activate", false ) end),
